...
class AppStore extends Component {
  ...
  getFilteredApps = () => {
    ...
    const filtered = appsList.filter(each => each.category === activeTabId)
    return filtered
  }
  ...
  render() {
    const {searchKey, activeTabId} = this.state
    console.log(searchKey)
    console.log(activeTabId)
    const filteredAppsList = this.getFilteredApps()
    const finalList = filteredAppsList.filter(everyItem =>
      everyItem.appName.toLowerCase().includes(searchKey.toLowerCase()),
    )
    return (
